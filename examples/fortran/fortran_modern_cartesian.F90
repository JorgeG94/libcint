!
! Modern Fortran example using iso_c_binding interface
! This demonstrates how to use libcint with type-safe interfaces
!
! Computes overlap gradient integrals for H2 molecule
!

program modern_cartesian
    use iso_c_binding
    ...
    ! ====================================================================
    ! Call one-electron integral (overlap gradient)
    ! Note: shell indices are 0-based in C
    ! ====================================================================
    print*, "Computing one-electron overlap gradient integral..."
    i = 0; shls(1) = i; di = CINTcgto_cart(i, bas)
    j = 1; shls(2) = j; dj = CINTcgto_cart(j, bas)

    allocate(buf1e(di, dj, 3))
    ret = cint1e_ipovlp_cart(buf1e, shls, atm, natm, bas, nbas, env)
    ...
    ! ====================================================================
    ! Call two-electron integral without optimizer
    ! ====================================================================
    print*, ""
    print*, "Computing two-electron gradient integral (no optimizer)..."
    i = 0; shls(1) = i; di = CINTcgto_cart(i, bas)
    j = 1; shls(2) = j; dj = CINTcgto_cart(j, bas)
    ...
    ! Pass c_null_ptr for no optimizer
    ret = cint2e_ip1_cart(buf2e, shls, atm, natm, bas, nbas, env, c_null_ptr)
    ...
    ! ====================================================================
    ! Call two-electron integral WITH optimizer
    ! ====================================================================
    print*, ""
    print*, "Computing two-electron gradient integral (with optimizer)..."
    ...
    allocate(buf2e(di, dj, dk, dl, 3))
    ret = cint2e_ip1_cart(buf2e, shls, atm, natm, bas, nbas, env, opt)
